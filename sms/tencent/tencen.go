...
import (
	...
	"github.com/baowk/dilu-plugin/sms/config"
...
func (t *TencentSms) Send(phone, code, tmpId string) error {
	// 实例化一个认证对象，入参需要传入腾讯云账户密钥对secretId,secretKey.
	credential := common.NewCredential(
		t.SmsConfig.SecretId,
		t.SmsConfig.SecretKey,
	)
	...
	request.SmsSdkAppId = common.StringPtr(t.SmsConfig.AppId)

	// 短信签名内容: 使用 UTF-8 编码，必须填写已审核通过的签名
	request.SignName = common.StringPtr(t.SmsConfig.Sign)

	/* 模板 ID: 必须填写已审核通过的模板 ID */
	request.TemplateId = common.StringPtr(tmpId)
